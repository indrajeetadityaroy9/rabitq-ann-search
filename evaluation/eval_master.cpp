...
int main(int argc, char** argv) {
    ...
    CPHNSWIndex<COMPILE_TIME_K> index(IndexParams().set_dim(sift.dim).set_M(32).set_ef_construction(200));
    Timer timer;
    timer.start();
    ...
    for (size_t ef : ef_values) {
        ...
        for (size_t q = 0; q < sift.num_queries; ++q) {
            timer.start();
            auto results = index.search(sift.query_vectors.data() + q * sift.dim, SearchParams().set_k(10).set_ef(ef).set_rerank(true, 200));
